...
export function LessonExperience({ module, lessons, exercises }: LessonExperienceProps) {
  ...
  return (
    <div className="mx-auto flex min-w-0 max-w-7xl flex-col gap-4 sm:gap-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <Link href="/dashboard" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-slate-900/70 px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:border-cyan-400/40 hover:text-white">
      ...
      <motion.header
        ...
        className="relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-slate-900/80 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.34)] backdrop-blur-2xl sm:p-6"
      >
        ...
        <div className="relative z-10 grid min-w-0 gap-4 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-6">
          <div className="min-w-0 max-w-3xl">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-300">Module #{module.ordre}</p>
                <h1 className="mt-1 text-2xl font-semibold tracking-[-0.03em] text-white sm:text-3xl">{module.titre}</h1>
              </div>
            </div>
          ...
          <PrimitiveCard tone="glass" className="p-4">
            ...
            <div className="mt-4 rounded-[1.1rem] border border-white/10 bg-slate-950/70 p-3">
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <SignalWaveIcon className="h-3 w-3 text-cyan-300" />
                <span>Signal de progression</span>
              </div>
              <p className="mt-1 text-xs leading-5 text-slate-300">Chaque section révèle un nouvel élément de compréhension, comme un chemin de signal parfaitement routé.</p>
            </div>
          </PrimitiveCard>
        </div>
      </motion.header>
        ...
        <aside className="space-y-5 xl:sticky xl:top-5 xl:self-start">
          <PrimitiveCard tone="glass" className="p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300">Sommaire</p>
            </div>
            <div className="mt-3 space-y-2">
              {lessons.map((lesson, index) => (
                <div key={lesson.id} className="rounded-[1rem] border border-white/10 bg-slate-950/70 p-2">
                  <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Section {index + 1}</p>
                  <p className="mt-1 text-xs font-semibold text-white">{lesson.titre}</p>
                </div>
              ))}
            </div>
          </PrimitiveCard>
          ...
          {lessons.map((lesson, index) => {
            ...
            return (
              <motion.section key={lesson.id} initial={shouldReduceMotion ? false : { opacity: 0, y: 16 }} whileInView={shouldReduceMotion ? { opacity: 1, y: 0 } : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.35, ease: 'easeOut' }}>
                <PrimitiveCard tone="floating" className="p-4 sm:p-7">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-300">Leçon {index + 1}</p>
                      <h2 className="mt-2 break-words text-xl font-semibold text-white sm:text-2xl">{lesson.titre}</h2>
                    </div>
